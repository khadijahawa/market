...
const ItemInfo = {
  ...
  description:
    "This is the second batch of Cybertino Genesis NFTs for early adopters and is 1 of 5. This Genesis NFT will be interactive: hold and wait for future exclusive benefits and early access to new drops!",
  wishlist: "1.3",
...
const ItemDetails = () => {
  ...
  const buyToken = async () => {
    ...
    const transactionParameters = {
      ...
      gas: ethers.BigNumber.from(300000).toHexString(),
      data: nftContract.methods
        .BULLSCMarketSaleToken(query.contactAddr, query.itemID)
        .encodeABI(), //make call to NFT smart contract
  ...
  const getTokenFromAddr = () => {
    ...
    const x = Tokens.filter(
      (key) => key.value.toLowerCase() == query?.token?.toString().toLowerCase()
    );
    console.log(x);
              ...
              <div className="col-lg-6">
                <div className="item-buy-part">
                  ...
                  <div className="item-price">
                    ...
                    <p>
                      <span>
                        <i className="icofont-coins"></i> {`${query.price}`}{" "}
                        {query?.isPayble == "true" ? " MATIC" : TokenLabel}
                      </span>
                    </p>
                  </div>
